...
import { Heart, ThumbsUp, ThumbsDown, Reply, MoreVertical, Edit, Trash2 } from 'lucide-react';
      ...
      <CardContent className="p-4">
        ...
        <div className="flex gap-4">
          <Avatar className="h-10 w-10">
            ...
            <AvatarFallback>{comment.userId.email[0].toUpperCase()}</AvatarFallback>
          </Avatar>
          <div className="flex-1">
            {/* Comment header */}
            <div className="flex items-center justify-between">
              <span className="font-medium text-gray-200">{comment.userId.email}</span>
              <div className="flex items-center gap-2">
                ...
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end" className="bg-black border-gray-700">
                    <DropdownMenuItem onClick={() => onEdit(comment._id, comment.comment)} className="text-gray-200 focus:bg-black focus:text-white">
                      <Edit className="mr-2 h-4 w-4" /> Edit
                    </DropdownMenuItem>
                    <DropdownMenuItem onClick={() => onDelete(comment._id)} className="text-gray-200 focus:bg-black focus:text-white">
                ...
                {comment.replies.map((reply) => (
                  <div key={reply._id} className="flex gap-4">
                    <Avatar className="h-8 w-8">
                      ...
                      <AvatarFallback>{reply.userId.email[0].toUpperCase()}</AvatarFallback>
                    </Avatar>
                    <div>
                      <div className="flex items-center gap-2">
                        <span className="font-medium text-gray-200">{reply.userId.email}</span>
                        <span className="text-sm text-gray-400">
